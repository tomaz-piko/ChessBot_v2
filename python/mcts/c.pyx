...
cpdef find_best_move(object board, Node root, object trt_func, unsigned int num_mcts_sims):
    ...
    if root.is_leaf():
        ...
        evaluate_node(root, policy_logits[0], board.legal_moves_tuple())
    ...
    while 1:
        ...
        while nodes_found < nodes_to_find and failsafe < 2:
            ...
            if terminal:
                ...
                update(root, tmp_board.moves_history(depth_to_root), flip_value(value))
                failsafe += 1
                continue
            ...
            moves_to_nodes.append(tmp_board.moves_history(depth_to_root))
...
cdef unsigned int select_child(Node node, unsigned int pb_c_base, float pb_c_init, float pb_c_factor, float fpu):
    ...
    for move, child in node.children.items():
        cN = child.N
        if cN > 0:
            ucb = UCB(cN, child.W, child.P, pN_sqrt, pb_c)
        else:
            ucb = fpu
        if ucb > bestucb:
            bestucb = ucb
...
@cython.wraparound(False)
@cython.initializedcheck(False)
cdef void evaluate_node(Node node, float[:] policy_logits, list legal_moves_tuple):
